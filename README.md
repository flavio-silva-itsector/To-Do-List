...
Check if noting beaked.

File names patterns:

- `MyComp.test.tsx` -> Unit tests; 
- `MyComp.spec.tsx` -> Integration tests; 

#### One time run

Just for check if all tests are passing run:
...
On development mode, to test on save files
